...
Download the [quickstart PDF](/files/v2/pdfdancer-v2-quickstart.pdf) as `input.pdf`. It contains `Hello` on page 1. The complete example below writes `output.pdf` with `Hello` replaced by `Final`.
