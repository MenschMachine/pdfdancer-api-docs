...
For required edits, accept the result only when the expected number of matches changed, `errors` is empty, and any warnings are acceptable. See [Text Layout](./text-layout) when text length changes.
