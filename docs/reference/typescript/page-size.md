---
id: page-size
title: "PageSize"
description: "TypeScript API reference for PageSize. Page dimensions in PDF points. This public interface belongs to the models package and documents API behavior used for PDF document editing and inspection in PDFDancer SDK 3.0.1."
pagination_next: null
pagination_prev: null
---

<!-- Generated by scripts/generate-v3-reference.js. Do not edit manually. -->

# `PageSize`

**Kind:** interface  
**Module/package:** `models`  
**SDK:** `pdfdancer-client-typescript 3.0.1`

Page dimensions in PDF points.

## Declaration

```typescript
interface PageSize
```

## Members

### `height`

```typescript
height?: number
```

### `name`

```typescript
name?: string
```

### `width`

```typescript
width?: number
```

## Related guides

- [Working with Pages](../../working-with-pages)

[View source at `f7b7f2d7c269`](https://github.com/MenschMachine/pdfdancer-client-typescript/blob/f7b7f2d7c26956f37cac21c00f7efd8a93883931/src/models.ts)

